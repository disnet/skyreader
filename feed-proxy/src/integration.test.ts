...
describe('Integration Tests', () => {
	...
	describe('GET /feed', () => {
		...
		it('filters items by since_guids', async () => {
			...
			const res = await app.request(
				'/feed?url=https://example.com/feed.xml&since_guids=guid-2',
				{ headers: { 'X-Proxy-Secret': 'test-secret' } }
			);
			const json = await res.json();
		...
		it('handles 304 Not Modified', async () => {
			...
			const cachedFeed = {
				title: 'Cached Blog',
				items: [{ guid: 'cached-1', url: '', title: 'Cached', publishedAt: new Date().toISOString() }],
				fetchedAt: Date.now() - 5000,
			};
			db.run(
				`INSERT INTO cache (url_hash, url, parsed_json, etag, cached_at, fetched_at) VALUES (?, ?, ?, ?, ?, ?)`,
				[urlHash, feedUrl, JSON.stringify(cachedFeed), '"etag-123"', Date.now() - 5000, Date.now() - 5000]
			);
		...
		it('stores and uses ETag for conditional requests', async () => {
			...
			db.run(
				`INSERT INTO cache (url_hash, url, parsed_json, etag, cached_at, fetched_at) VALUES (?, ?, ?, ?, ?, ?)`,
				[urlHash, feedUrl, JSON.stringify(cachedFeed), '"test-etag-123"', Date.now() - 5000, Date.now() - 5000]
			);

			// Mock fetch to capture the request headers
			let requestHeaders: Headers | undefined;
			fetchMock = spyOn(globalThis, 'fetch').mockImplementation((async (url: unknown, init?: RequestInit) => {
	...
	describe('Cache behavior', () => {
		it('serves stale cache while refreshing', async () => {
			...
			const cachedFeed = {
				title: 'Stale Feed',
				items: [{ guid: 'stale-1', url: '', title: 'Stale Post', publishedAt: new Date().toISOString() }],
				fetchedAt: Date.now() - 5000,
			};
			db.run(
				`INSERT INTO cache (url_hash, url, parsed_json, cached_at, fetched_at) VALUES (?, ?, ?, ?, ?)`,
	...
	describe('cleanupCache', () => {
		it('removes entries older than 7 days', () => {
			...
			const remaining = db.query<{ count: number }, []>('SELECT COUNT(*) as count FROM cache').get();
	...
	describe('filterItems', () => {
		...
		it('returns empty array when latest GUID matches', async () => {
			...
			const res = await app.request(
				'/feed?url=https://example.com/feed.xml&since_guids=guid-3',
				{ headers: { 'X-Proxy-Secret': 'test-secret' } }
			);
			const json = await res.json();
	...
	describe('Error handling and backoff', () => {
		...
		it('increments error count on subsequent failures', async () => {
			...
			const first = db.query<CacheRow, [string]>('SELECT * FROM cache WHERE url_hash = ?').get(urlHash);
			...
			db.run('UPDATE cache SET next_retry_at = NULL, fetched_at = ? WHERE url_hash = ?', [Date.now() - 10000, urlHash]);
			...
			const second = db.query<CacheRow, [string]>('SELECT * FROM cache WHERE url_hash = ?').get(urlHash);
		...
		it('skips fetch when in backoff period (circuit breaker)', async () => {
			...
			const cachedFeed = {
				title: 'Backoff Feed',
				items: [{ guid: 'backoff-1', url: '', title: 'Post', publishedAt: new Date().toISOString() }],
				fetchedAt: Date.now(),
			};
			db.run(
				`INSERT INTO cache (url_hash, url, parsed_json, cached_at, fetched_at, error_count, last_error, next_retry_at)
				VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
				[urlHash, feedUrl, JSON.stringify(cachedFeed), Date.now(), Date.now(), 3, 'HTTP 500', Date.now() + 60000]
			);
		...
		it('resets error count on successful fetch', async () => {
			...
			db.run(
				`INSERT INTO cache (url_hash, url, parsed_json, cached_at, fetched_at, error_count, last_error, next_retry_at)
				VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
				[urlHash, feedUrl, JSON.stringify(cachedFeed), Date.now() - 10000, Date.now() - 10000, 3, 'HTTP 500', Date.now() - 1000] // backoff expired
			);
		...
		it('includes error stats in /stats endpoint', async () => {
			...
			db.run(
				`INSERT INTO cache (url_hash, url, parsed_json, cached_at, fetched_at, error_count, last_error, next_retry_at)
				VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
				['error1', 'https://example.com/err1', '{}', Date.now(), Date.now(), 2, 'HTTP 500', Date.now() + 60000]
			);
			db.run(
				`INSERT INTO cache (url_hash, url, parsed_json, cached_at, fetched_at, error_count, last_error, next_retry_at)
				VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
				['error2', 'https://example.com/err2', '{}', Date.now(), Date.now(), 5, 'HTTP 403', Date.now() + 7 * 24 * 60 * 60 * 1000]
			);
			db.run(
		...
		it('returns error info in bulk feed response for real content with errors', async () => {
			...
			const realFeed = {
				title: 'Real Feed',
				items: [{ guid: 'item-1', url: '', title: 'Post', publishedAt: new Date().toISOString() }],
				fetchedAt: Date.now() - 1000,
			};
			db.run(
				`INSERT INTO cache (url_hash, url, parsed_json, cached_at, fetched_at, error_count, last_error, last_error_at, next_retry_at)
				VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
				[urlHash, feedUrl, JSON.stringify(realFeed), Date.now() - 1000, Date.now() - 1000, 3, 'HTTP 500', Date.now() - 1000, Date.now() - 1] // backoff expired
			);
	...
	describe('Cached error placeholder handling', () => {
		describe('GET /feed - empty placeholder errors', () => {
			...
			it('serves stale content when feed had real content before errors', async () => {
				...
				const realFeed = {
					...
					items: [
						{ guid: 'item-1', url: 'https://example.com/1', title: 'Real Post', publishedAt: new Date().toISOString() },
					],
					fetchedAt: Date.now(),
				};
				db.run(
					`INSERT INTO cache (url_hash, url, parsed_json, cached_at, fetched_at, error_count, last_error, next_retry_at)
					VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
					[urlHash, feedUrl, JSON.stringify(realFeed), Date.now(), Date.now(), 3, 'HTTP 500', Date.now() + 60000]
				);
			...
			it('distinguishes empty title with items from true empty placeholder', async () => {
				...
				const feedWithEmptyTitle = {
					title: '',
					items: [
						{ guid: 'item-1', url: 'https://example.com/1', title: 'Post', publishedAt: new Date().toISOString() },
					],
					fetchedAt: Date.now(),
				};
				db.run(
					`INSERT INTO cache (url_hash, url, parsed_json, cached_at, fetched_at, error_count, last_error, next_retry_at)
					VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
					[urlHash, feedUrl, JSON.stringify(feedWithEmptyTitle), Date.now(), Date.now(), 2, 'HTTP 500', Date.now() + 60000]
				);
			...
			it('distinguishes feed with title but no items from true empty placeholder', async () => {
				...
				db.run(
					`INSERT INTO cache (url_hash, url, parsed_json, cached_at, fetched_at, error_count, last_error, next_retry_at)
					VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
					[urlHash, feedUrl, JSON.stringify(feedWithTitleNoItems), Date.now(), Date.now(), 2, 'HTTP 500', Date.now() + 60000]
				);
		...
		describe('POST /feeds - empty placeholder errors', () => {
			...
			it('handles mix of working feeds and error placeholders', async () => {
				...
				db.run(
					`INSERT INTO cache (url_hash, url, parsed_json, cached_at, fetched_at, error_count, last_error, next_retry_at)
					VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
					[errorUrlHash, errorUrl, JSON.stringify(emptyFeed), Date.now(), Date.now(), 1, 'HTTP 404', Date.now() + 60000]
				);
			...
			it('serves stale content in bulk when feed had real content before errors', async () => {
				...
				const realFeed = {
					...
					items: [
						{ guid: 'bulk-1', url: 'https://example.com/1', title: 'Bulk Post', publishedAt: new Date().toISOString() },
					],
					fetchedAt: Date.now(),
				};
				db.run(
			...
			it('returns ERROR for multiple error placeholder feeds in single request', async () => {
				...
				for (const url of urls) {
					...
					db.run(
						`INSERT INTO cache (url_hash, url, parsed_json, cached_at, fetched_at, error_count, last_error, next_retry_at)
						VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
						[urlHash, url, JSON.stringify(emptyFeed), Date.now(), Date.now(), 1, 'HTTP 404', Date.now() + 60000]
					);
				}
		...
		describe('Edge cases', () => {
			it('returns ERROR when backoff active even if cache age would be STALE', async () => {
				...
				const emptyFeed = { title: '', items: [], fetchedAt: Date.now() - 5000 }; // 5 seconds ago
				db.run(
					`INSERT INTO cache (url_hash, url, parsed_json, cached_at, fetched_at, error_count, last_error, next_retry_at)
					VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
					[urlHash, feedUrl, JSON.stringify(emptyFeed), Date.now() - 5000, Date.now() - 5000, 1, 'HTTP 404', Date.now() + 60000]
				);
			...
			it('retries and creates new placeholder after backoff expires', async () => {
				...
				const emptyFeed = { title: '', items: [], fetchedAt: Date.now() - 5000 };
				db.run(
					`INSERT INTO cache (url_hash, url, parsed_json, cached_at, fetched_at, error_count, last_error, next_retry_at)
					VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
					[urlHash, feedUrl, JSON.stringify(emptyFeed), Date.now() - 5000, Date.now() - 5000, 1, 'HTTP 404', Date.now() - 1000] // expired
				);
			...
			it('recovers to HIT when feed starts working after errors', async () => {
				...
				db.run(
					`INSERT INTO cache (url_hash, url, parsed_json, cached_at, fetched_at, error_count, last_error, next_retry_at)
					VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
					[urlHash, feedUrl, JSON.stringify(emptyFeed), Date.now() - 5000, Date.now() - 5000, 3, 'HTTP 404', Date.now() - 1000] // expired
				);
		...
		describe('GET /discover', () => {
			...
			it('returns the URL for atom content-type', async () => {
				const { app } = createTestApp();
				fetchMock = mockFetch(() => new Response('<feed></feed>', {
			...
			it('returns empty array when no feeds found', async () => {
				...
				fetchMock = spyOn(globalThis, 'fetch').mockImplementation((async (_url: unknown, init?: RequestInit) => {
					if (init?.method === 'HEAD') {
						return new Response('Not Found', { status: 404 });
					}
					return new Response(html, {
		...
		describe('Response size limits', () => {
			...
			it('rejects responses over the size limit via Content-Length header', async () => {
				...
				fetchMock = mockFetch(() => new Response('small body', {
			...
			it('returns cached content when new fetch exceeds size limit', async () => {
				...
				const cachedFeed = {
					title: 'Good Feed',
					items: [{ guid: 'item-1', url: 'https://example.com/1', title: 'Post', publishedAt: new Date().toISOString() }],
					fetchedAt: Date.now() - 10000,
				};
				db.run(
					`INSERT INTO cache (url_hash, url, parsed_json, cached_at, fetched_at, error_count)
