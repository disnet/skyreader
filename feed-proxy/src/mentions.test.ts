...
function mockConstellation(
	...
	return spyOn(globalThis, 'fetch').mockImplementation((async (
		...
		if (url.pathname === '/links') {
			...
			return new Response(
				JSON.stringify({ total: linking_records.length, linking_records }),
			);
		}
...
describe('normalizeArticleUrl', () => {
	it('strips tracking params, trailing slash, and fragment; lowercases host; keeps www', () => {
		expect(
			normalizeArticleUrl(
				'https://www.Example.com/post/?utm_source=x&id=7#section',
			),
		).toBe('https://www.example.com/post?id=7');
	});
...
describe('laneForSource', () => {
	it('buckets known collections and ignores excluded paths', () => {
		expect(laneForSource('site.standard.document', '.links[].uri')?.id).toBe(
			'linkblog',
		);
		expect(laneForSource('app.bsky.feed.post', '.embed.external.uri')?.id).toBe(
			'bluesky',
		);
		expect(laneForSource('at.margin.note', '.target.source')?.id).toBe(
			'margin',
		);
		expect(laneForSource('network.cosmik.card', '.content.url')?.id).toBe(
			'semble',
		);
		...
		expect(
			laneForSource('app.bsky.feed.post', '.embed.images[].alt'),
		).toBeNull();
	});
	...
	it('ignores un-laned collections (referrer/analytics noise)', () => {
		expect(
			laneForSource(
				...
				'.sessionContext.referrer',
			),
		).toBeNull();
		expect(
			laneForSource('community.lexicon.bookmarks.bookmark', '.subject'),
		).toBeNull();
	});
});
