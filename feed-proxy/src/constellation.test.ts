...
function mockConstellationFetch(opts: {
		...
		if (url.startsWith('https://plc.directory/')) {
			return new Response(
				JSON.stringify({
					...
					service: [
						{ id: '#atproto_pds', type: 'AtprotoPersonalDataServer', serviceEndpoint: LINKER_PDS },
					],
				})
			);
		}
					...
					value: {
						...
						content: {
							...
							pages: [
								{
									blocks: [
										{ block: { $type: 'pub.leaflet.blocks.text', plaintext: opts.note ?? 'great read' } },
									],
								},
							],
						},
...
describe('getSocialContext', () => {
	...
	it('assembles counts + also-linked-by with resolved handles and notes', async () => {
		const { db } = createTestApp();
		mockConstellationFetch({ recommendCount: 3, quoteCount: 1, linkingDids: [LINKER], note: 'love this' });

		const ctx = await getSocialContext(db, { docUri: DOC_URI, articleUrl: ARTICLE });
	...
	it('serves a cached bundle on the second call (no extra fetches)', async () => {
		const { db } = createTestApp();
		const spy = mockConstellationFetch({ recommendCount: 5, articleUrl: ARTICLE } as never);
...
describe('POST /social-context', () => {
	...
	it('returns a per-item context keyed back to the request', async () => {
		...
		const res = await post(app, { items: [{ key: 'a', docUri: DOC_URI, articleUrl: ARTICLE }] });
		expect(res.status).toBe(200);
		const json = (await res.json()) as { items: Array<{ key: string; recommendCount: number }> };
