<script lang="ts">
  ...
  const errorMessages: Record<string, string> = {
    ...
    'temporarily_unavailable': 'The authentication server is temporarily unavailable. Please try again later.',
    'session_expired': 'Your session has expired. Please log in again.',
    'callback_failed': 'Authentication callback failed. Please try again.',
  };

  const errorCode = $derived($page.url.searchParams.get('error') || 'unknown');
  const error = $derived(errorMessages[errorCode] || 'An unexpected error occurred. Please try again.');
</script>
