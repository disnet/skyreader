...
export async function handleDeleteShareDraft(request: Request, env: Env): Promise<Response> {
  ...
  try {
    ...
    //
    // An upsert, not an UPDATE: deleting a draft the server has never seen must
    // still leave a clock-stamped tombstone. Otherwise an older write still in
    // flight — this device's offline queue, or another device's — lands on an
    // empty key, inserts a live row, and the posted or discarded draft comes
    // back everywhere.
    await env.DB.prepare(
      `INSERT INTO share_drafts (user_did, article_url, draft, client_updated_at, created_at, updated_at, deleted_at)
       VALUES (?, ?, '{}', ?, ?, ?, ?)
       ON CONFLICT(user_did, article_url) DO UPDATE SET
         draft = '{}',
         client_updated_at = excluded.client_updated_at,
      ...
      .bind(session.did, body.articleUrl, clientUpdatedAt, now, now, now)
      .run();
