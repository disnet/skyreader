...
// These forward to the new unified endpoints

// GET /api/social/share-read - Get share read positions (legacy)
export async function handleGetShareReadPositions(request: Request, env: Env): Promise<Response> {
  if (request.method !== 'GET') {
    return new Response(JSON.stringify({ error: 'Method not allowed' }), {
      status: 405,
      headers: { 'Content-Type': 'application/json' },
  ...
  try {
    const result = await env.DB.prepare(
      `SELECT rkey, item_uri, author_did, item_url, item_title, read_at
			 FROM social_read_positions_cache
			 WHERE user_did = ? AND item_type = 'share'`
    )
      .bind(session.did)
      .all<{
        rkey: string;
    ...
    return new Response(
      JSON.stringify({
        positions: result.results.map((row) => ({
          rkey: row.rkey,
          shareUri: row.item_uri,
          shareAuthorDid: row.author_did,
          itemUrl: row.item_url,
          itemTitle: row.item_title,
          readAt: row.read_at,
        })),
    ...
    console.error('Get share read positions error:', error);
    const errorMessage =
      error instanceof Error
        ? `${error.name}: ${error.message}`
        : 'Failed to get share read positions';
    return new Response(JSON.stringify({ error: errorMessage }), {
      status: 500,
      headers: { 'Content-Type': 'application/json' },
    });
...
// POST /api/social/share-read - Mark a share as read (legacy)
export async function handleMarkShareAsRead(request: Request, env: Env): Promise<Response> {
  if (request.method !== 'POST') {
    return new Response(JSON.stringify({ error: 'Method not allowed' }), {
      status: 405,
      headers: { 'Content-Type': 'application/json' },
  ...
  const { rkey, shareUri, shareAuthorDid, itemUrl, itemTitle } = body;
  ...
  if (!shareUri || typeof shareUri !== 'string') {
    return new Response(JSON.stringify({ error: 'shareUri is required' }), {
      status: 400,
      headers: { 'Content-Type': 'application/json' },
    });
  }

  if (!isValidAtUri(shareUri)) {
    return new Response(JSON.stringify({ error: 'shareUri must be a valid AT URI' }), {
      status: 400,
      headers: { 'Content-Type': 'application/json' },
    });
  }

  if (!shareAuthorDid || typeof shareAuthorDid !== 'string') {
    return new Response(JSON.stringify({ error: 'shareAuthorDid is required' }), {
      status: 400,
      headers: { 'Content-Type': 'application/json' },
    });
  }

  if (!isValidDid(shareAuthorDid)) {
    return new Response(JSON.stringify({ error: 'shareAuthorDid must be a valid DID' }), {
      status: 400,
      headers: { 'Content-Type': 'application/json' },
    });
  }
  ...
  try {
    ...
    console.error('Mark share as read error:', error);
    const errorMessage =
      error instanceof Error ? `${error.name}: ${error.message}` : 'Failed to mark share as read';
    return new Response(JSON.stringify({ error: errorMessage }), {
      status: 500,
      headers: { 'Content-Type': 'application/json' },
    });
...
// DELETE /api/social/share-read/:rkey - Remove share read position (legacy)
export async function handleMarkShareAsUnread(request: Request, env: Env): Promise<Response> {
  if (request.method !== 'DELETE') {
    return new Response(JSON.stringify({ error: 'Method not allowed' }), {
      status: 405,
      headers: { 'Content-Type': 'application/json' },
  ...
  try {
    ...
    console.error('Mark share as unread error:', error);
    const errorMessage =
      error instanceof Error ? `${error.name}: ${error.message}` : 'Failed to mark share as unread';
    return new Response(JSON.stringify({ error: errorMessage }), {
      status: 500,
      headers: { 'Content-Type': 'application/json' },
    });
