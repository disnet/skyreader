...
export async function getItemsGeneration(env: Env): Promise<string> {
...
export async function handleTimeline(
  ...
  const generation = await getItemsGeneration(env);
  ...
  const incremental = sinceSeq !== undefined && generationParam === generation && generation !== '';
  ...
  try {
    ...
    // Cold start: no cursor, or a generation mismatch (D1 recreated / restored).
    ...
    const rows: ItemRow[] = [];
    for (let i = 0; i < feedUrls.length; i += COLD_START_CHUNK) {
      const statements = feedUrls.slice(i, i + COLD_START_CHUNK).map((feedUrl) =>
        env.DB.prepare(
          `SELECT fi.seq, fi.feed_url, fi.item_json, ${READ_FLAG_SQL}
             FROM feed_items fi
            WHERE fi.feed_url = ?2
    ...
    let cursor = 0;
    for (const row of rows) if (row.seq > cursor) cursor = row.seq;
    if (cursor === 0) {
      // Nothing to deliver (fresh account, or the feeds aren't ingested yet).
      // Start the client at the current head rather than 0, so its first
      // incremental poll doesn't scan the archive from the beginning. Anything
      // ingested from here on is above this cursor; history for a brand-new
      // subscription arrives via the single-feed endpoint's pull-through.
      const head = await env.DB.prepare('SELECT MAX(seq) AS max_seq FROM feed_items').first<{
        max_seq: number | null;
      }>();
      cursor = head?.max_seq ?? 0;
    }
