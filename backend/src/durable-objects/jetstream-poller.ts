...
interface JetstreamEvent {
  ...
  commit?: {
    ...
    record?: {
      ...
      // For shares: string content; For documents: structured content object
      content?: string | { $type: string; pages?: unknown[] };
      itemImage?: string;
      itemGuid?: string;
...
export class JetstreamPoller implements DurableObject {
  ...
  async alarm(): Promise<void> {
    ...
    try {
      ...
      try {
        ...
        // 2. Subscriptions (app.skyreader.feed.subscription) - global watch, filter by sync-enabled
        ...
        // 3. Documents (site.standard.document) - filter to followed users
        ...
        // 4. Saved (app.skyreader.feed.saved) - global watch, filter by registered users
      ...
      console.log(
        `[JetstreamPoller] Poll complete: shares=${stats.shares.processed}/${stats.shares.errors}, ` +
  ...
  private async pollSharesStream(): Promise<{ processed: number; errors: number }> {
    ...
    return new Promise((resolve) => {
      ...
      const cleanupWithCatch = () =>
        cleanup().catch((e) => console.error('[JetstreamPoller] shares cleanup error:', e));
      ...
      try {
        ...
        ws.addEventListener('message', async (event) => {
          try {
            ...
            if (
              data.kind === 'commit' &&
              data.commit?.collection === 'app.skyreader.social.share'
            ) {
  ...
  private async processShareEvent(event: JetstreamEvent): Promise<void> {
    ...
    if (operation === 'create' && record && cid) {
      // Ensure user exists first (for FK constraint on shares.author_did)
      // Minimal data - profile fetched on-demand when displaying
      try {
      ...
      // Insert share into D1 - content comes directly from the record
      // Use ON CONFLICT to preserve reshare_count if the record already exists
      // (handleCreateShare may have inserted it milliseconds before)
      try {
        await this.env.DB.prepare(
          `
					INSERT INTO shares
					...
					ON CONFLICT(record_uri) DO UPDATE SET
						...
						reshare_of_uri = COALESCE(shares.reshare_of_uri, excluded.reshare_of_uri),
						reshare_of_author_did = COALESCE(shares.reshare_of_author_did, excluded.reshare_of_author_did),
						reshare_count = shares.reshare_count
					`
        )
          .bind(
            did,
        ...
        console.error(`[JetstreamPoller] D1 WRITE ERROR inserting share for ${did}:`, dbError);
        throw dbError;
      }
    } else if (operation === 'delete') {
      try {
        await this.env.DB.prepare('DELETE FROM shares WHERE record_uri = ?').bind(recordUri).run();
      } catch (dbError) {
        console.error(`[JetstreamPoller] D1 WRITE ERROR deleting share ${recordUri}:`, dbError);
        throw dbError;
      }
    }
  }
