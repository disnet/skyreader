...
describe('/api/linkblog/drafts', () => {
  ...
  it('deleting a never-pushed draft blocks an older write that lands later', async () => {
    // Offline draft queued at t=1000, then posted online at t=2000 before the
    // queue drained: the delete reaches the server first, the stale upsert after.
    await mutate('DELETE', { articleUrl: 'https://example.com/a', updatedAt: 2000 });
    await mutate('PUT', {
      articleUrl: 'https://example.com/a',
      draft: draftBody('queued offline'),
      updatedAt: 1000,
    });
    ...
    // And the tombstone replays to other devices.
    const delta = await getDrafts('/api/linkblog/drafts?since=0');
    expect(delta.body.drafts).toHaveLength(1);
    expect(delta.body.drafts[0].deletedAt).not.toBeNull();
    expect(delta.body.drafts[0].draft).toBeNull();
  });
