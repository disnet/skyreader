...
function createShareDraftsStore() {
  ...
  async function remove(articleUrl: string) {
    ...
    // An upsert still in the offline queue predates this delete. The server's
    // tombstone would reject it, but it has no reason to go out at all.
    await syncQueue.cancelPending('shareDraft', articleUrl);
    try {
