...
const { dbRows, metadata, api, enqueue, cancelPending, online, authed } = vi.hoisted(() => ({
...
describe('shareDraftsStore', () => {
  ...
  it('discards the pending write when the draft is removed', async () => {
    ...
    try {
      ...
      // An upsert parked in the offline queue is dropped along with the timer.
      expect(cancelPending).toHaveBeenCalledWith('shareDraft', 'https://a.example/');
      await vi.runAllTimersAsync();
      expect(api.upsertShareDraft).not.toHaveBeenCalled();
      expect(store.list).toHaveLength(0);
    } finally {
