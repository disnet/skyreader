...
// via the backend (`/api/v2/social-context`): how many people recommended it, how
// many quoted it, and who else across the Atmosphere linked the same article
// (with their notes). This is *adornment only* — it loads lazily when a link post
// is opened and degrades silently (no line shown) if Constellation is unavailable.
//
// Session-scoped, in-memory memo keyed by the link post's record URI. The proxy
// caches the expensive Constellation/PDS work, so a per-session map is enough.
...
function createSocialContextStore() {
  ...
  async function fetch({ docUri, articleUrl, excludeDid }: FetchArgs) {
    ...
    try {
      const res = await api.fetchSocialContext([{ key: docUri, docUri, articleUrl, excludeDid }]);
