    ...
    $effect(() => {
        ...
        if (
            ...
            if (feedFilter) {
                ...
                let feedArticles = currentArticles.filter(
                    (a) => a.subscriptionId === feedId,
                );
                ...
                filtered = currentArticles.filter(
                    (a) => readPositions.get(a.guid)?.starred ?? false,
                );
            } else {
        ...
        {:else if currentItems.length === 0}
            ...
            {:else if sharedFilter}
                <EmptyState
                    ...
                    description="Share articles to see them here"
                />
            ...
            {:else if feedFilter}
                ...
                {:else}
                    <EmptyState
                        title="No articles"
                        description="This feed has no articles"
                    />
                {/if}
            {:else if feedsFilter}
                <EmptyState
                    title="No unread articles"
                    description="You're all caught up on your feeds"
                />
                    ...
                    <div bind:this={articleElements[index]}>
                        ...
                        {:else}
                            ...
                            {@const isFetching = share.itemGuid
                                ? fetchingArticles.has(share.itemGuid)
                                : false}
                            <ShareCard
                                {share}
                                {localArticle}
                                ...
                                onFetchContent={() => {
                                    const hasLocalContent =
                                        localArticle?.content || localArticle?.summary;
